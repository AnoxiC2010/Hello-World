...
MULTI开启多命令模式，EXEC执行所有队列里的命令。

你可以将Redis中的事务就理解为 ：Redis事务提供了⼀种将多个命令请求打包的功能。然后，
再按顺序执⾏打包的所有命令，并且不会被中途打断。



不支持roll back，官方认为没有必要让redis的性能为程序员的错误买单。
...
1. 参数检查（拦截无效参数、比如格式、范围）
2. 缓存无效key，设置短一点的过期时间，防止黑客变换无效key攻击造成大量内存占用
3. 布隆过滤器
...
库存扣减成功，producer的executeLocalTransaction方法返回COMMIT_MESSAGE，但是如果网络问题等因素导致MQ服务器没收到，的话，MQ会通过Listener中的checkLocalTransaction方法回查，为了防止重复扣减库存，在扣减库存成功时以事务消息的transactionId为key的组成部分把扣减结果放在redis中，回查的时候首先检查redis中了解扣减情况，如果扣减则直接返回COMMIT_MESSAGE让MQ服务器通知consumer执行分布式事务的第二阶段，如果内有扣减失败则返回ROLLBACK_MESSAGE结束本次事务。

分布式事务2阶段

二阶段同样存在幂等性问题，比如创建成功了订单，但是返回消息因为网络波动MQ服务器没有收到，MQ服务器会尝试16次知道cunsumer返回CONSUME_SUCCESS，但是为了防止订单重复创建，也通过redis把事务消息的transactionId作为key的一部分把这一阶段事务的执行结果放入redis，在创建订单之前需要检查redis中的结果，如果发现已经创建了就直接返回CONSUME_SUCCESS，不继续往下走了。RECONSUME_LATER一般在数据库出错没有成功创建订单的情况下会返回这个值。

当然由于orderId的唯一性，也可以作为在redis中key值的组成部分，同时其唯一性也可以作为幂等性的保障，因为数据库不能重复插入，但是使用redis缓存作为幂等性保障可以减少数据库不必要的连接压力。
...
![image-20210721183118697](C:\Users\AnoxiC2010\Documents\GitHub\Hello-World\Java\Java-QA.assets\image-20210721183118697.png)

秒杀流程：第一步扣减库存，第二步创建订单，一开始将热点数据放入redis里面，这个时候会产生新的问题就是在高并发情况下redis的库存和DB库存数据不一致，那么我在这里采用了RocketMQ的分布式事务来保证他们的数据的最终一致性。正因为访问量很大，这里为了接口能承受高并发的压力，我做了两个优化

①缓存，我将商品的热点信息放置本地缓存，访问时在本地缓存直接取出来，因为它是存在于tomcat本地应用中，不经过IO流所以会很快。
②限流，主要由三种：redis拦截，令牌桶，队列泄洪，
全部打在redis上，在库存没有的时候，redis 就存空库存标记，
全部拦截
RateLimiter令牌桶，只有拿到令牌的才能继续执行请求，没拿到的就需要等待

做一个队列泄洪，创建一个具有固定数量的线程池,任务缓存队列，把请求交给线程池，让他在线程池中进行，多出来的自动缓存在线程池任务队列里

开始扣减库存，使用了recketMq事务消息保证最终一致性，扣减库存成功之后，消费者才能收到消息，
消费者收到了事务消息才会创建订单的时候，分布式锁，锁住扣减库存的原子操作，//提一下redis淘汰策略，全部淘汰

创建订单的时候，消费者消费消息才创建订单，消费者收到消息说明库存以及扣减了，消费者创建订单也是原子操作，失败会16次，16次之后会放入死性人工干预，手动拿出来消费。

如果库存减了，订单未成功，不能回滚，只能人工干预
③降级，将核心业务保留，次要业务降级成只读不写或者关闭次要业务。只有618大促的时候
...
为什么要分布式事务（里面使用的分布式锁）:
 纯业务层面的秒杀下单实现，有没有什么隐含的问题呢？

1. 如果我们有多个秒杀服务，多个秒杀服务同时接收对同一个商品的秒杀下单的请求，此时就出现了并发的秒杀下单请求就有可能出现，超卖问题, 如何解决呢？ 加锁 ——> 多并发请求的同步

但是，synchronized是没有意义的，因为synchronized所使用的锁，进程级的，但是不同的服务是不同的进程，对于不同的进程无法使用synchronized锁对象实现同步 ——> 分布式锁

2. 秒杀下单，对于一次下单请求，要么成功，要么失败？ 所以从这个角度讲，我们希望一次秒杀下单过程应该是一个事物如果仅仅只是在方法上加 @Transactional注解，是不能将秒杀下单的功能作为一个事物的，因为秒杀服务和订单服务的sql语句是在不同的数据库连接中去发送和执行的，原理如下

       1. 获取连接

         2. 开启事物  connection.setAutoCommit(false)
            // 扣减库存
            // 保存订单信息
            // 保存订单邮寄消费
     ...
     所以，常规的事物，在这里就不生效了， 如何将不同服务对数据库的访问变成一个事物呢？  ——> 分布式式事物
...
![image-20210719214631662](C:\Users\AnoxiC2010\Documents\GitHub\Hello-World\Java\Java-QA.assets\image-20210719214631662.png)

## 项目三购物车

来自：王富强

0、有用redis做购物车，那这个map的key是怎么设计的（数据库名字是用的"cart"+用户id，里面hash的key是CART_ITEM_CACHE_PREFIX (自定义的)+ ":" + itemId）、

存在map里的商品信息会有过期时间吗、

如果单个redis内存不够用又该怎么处理
Key value
--------购物车的数据如何存在数据库表里 隔一段时间同步到数据库 过期时间，过期时间的末尾，同步到数据库



0、如何防止超卖？
如悲观锁，分布式锁，乐观锁，队列串行化，异步队列分散，Redis原子操作

实际开发的时候，查询购物车的时候，实时调用别的服务，比如价格库存之类的查询最新数据
