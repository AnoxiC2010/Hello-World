...
注意servlet-api的scope范围设置为provided，应为服务器环境下应有tomcat的依赖存在了
...
很重要的一个技能。SE、DS、DB、EE   几个按钮   博客 IDEA去debug

做项目的时候肯定会遇到bug，数据库插入不成功、修改不成功、没有报错。debug

debug：

**就是在程序执行的通路上面设置一个挡板，那么就可以把程序给拦着**

打一个断点，不会触发debug，只有你的程序运行起来，才会触发断点



程序执行流程：

入口servlet的service方法，doGet或者doPost----------->（step into）  service--------------

**千万不要守株待兔（打个断点，等待程序从天而降）**
...
step out：从方法中出来，回到调用该方法的地方

resume program：继续放行代码，如果后面有断点，会直接调到下一个断点；如果没有断点，那么会直接全部执行完

细心



后台维护前台用户系统所下的订单。查询订单，根据不同的条件进行查询

后台中并没有订单的生成入口。生成入口是在前台，后台管理系统手动先插入数据，来模拟

编辑：首先进行回显。订单的规格、数量不要修改，否则逻辑会非常混乱，只去修改订单的状态即可
...
本项目中加入购物车的商品其实就是生成了一个订单，此时订单的状态是未付款

直接点击购买，那么是下了一个已付款的订单

本项目中没有引入支付

订单中永远只有一个商品，所以表的设计简单很多。

本项目中也没有多收货地址的概念，所以呢，收货地址也不需要单独去新建一个表
...
createtime----订单表强烈建议设置一个时间字段，很多的统计；双十一 多少秒破亿；一天24h的销售额；多长时间持平去年全天

updatetime---统计订单的更新速度。商家的订单响应速度

hasComment---当前订单是否已经评价



新建任何一个Java对象，最好顺手就把get和set方法给新建了

很多的框架在读取数据、封装数据的时候都会使用get和set方法来操作
...
![image-20210518102534487](C:\Users\AnoxiC2010\Desktop\wdJava30th\EE\Day12 订单模块\项目一.assets\image-20210518102534487.png)
...
用户使用的前台系统也是需要进行拦截的，拦截的页面有哪些呢？和用户的个人信息息息相关的，比如个人主页、加入购物车、下单等

如果希望访问这些页面，那么需要进行登录（指的是用户的登录，不是后台管理系统的管理员）
...
localhost来访问肯定不行了，192.168.7.xx

需要更改如下设置：

**axios-admin.js以及axios-client.js文件中的baseURL**

**同时filter中Access-Control-Allow-Origin    192.168.7.xx:8085**

设置完成之后同桌之间相关访问



如果访问不到，尝试将防火墙关闭，火绒卸载
